...
//
// To execute
// jsonnet -J vendor -S -e 'std.manifestYamlDoc((import "jsonnet/grafanaOperationalDashboard.jsonnet"))' --ext-code "values={ kube_x: std.parseYaml(importstr \"../../kube-x/values.yaml\") }"
//
// By default, the folder is the namespace name
{
  ...
  metadata: {
    name: 'argocd-mixin-operational-dashboard'
  },
  spec:
    {
      ...
      json: std.manifestJson(
            certManager {
             ...
             }.grafanaDashboards["argo-cd-operational-overview.json"]
             ),
    },
}
